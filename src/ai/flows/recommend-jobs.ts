...
const RecommendJobsInputSchema = z.object({
  ...
  studentGPA: z.number().describe('The GPA of the student.'),
  ...
  allJobs: z.array(z.object({
    ...
    requiredGPA: z.number().describe('The minimum GPA required for the job.'),
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an AI job recommendation system. You will be provided with a student's skills, GPA, past applications, and a list of all available jobs. Your task is to recommend the jobs that are most relevant to the student.
...
For each job, consider the student's skills, GPA, and past applications to determine its relevance. A higher GPA and a greater overlap between the student's skills and the job's required skills should result in a higher relevance score. Also, consider that the student might not want to see jobs that they already applied to. 
